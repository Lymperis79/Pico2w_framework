...
class ConfigManager:
    def __init__(self):
        self.data = {'sensor_interval_ms': app.SENSOR_INTERVAL_MS}
        self.load()
    ...
    def sensor_interval_ms(self):
        return int(self.data['sensor_interval_ms'])

    def apply(self, d):
        if 'sensor_interval_ms' in d:
            v = int(d['sensor_interval_ms'])
            if v < 100 or v > 86400000:
                raise ValueError('invalid sensor interval')
            self.data['sensor_interval_ms'] = v
            self.save()
